...
export const AddCartProductToLocalStorage = (product:ProductType,currentColor:string,currentSize:string,quanity:number) => {
    ...
    const id = uuidv4()
    localStorage.setItem(id,JSON.stringify({...product,id,type:'ave-product',currentColor,currentSize,quanity,productId:product.id}))
}
                        ...
                        <div className="product_content__item">
                            ...
                            <div className="product_content__item__actions">
                                <ul className="product_content__item__rating">
                                    {Array(product.rating).fill(1).map((rate,index) => <li key={index} className={'rate'}>
                                        <StarRate />
                                    </li>)}
                    ...
                    <div className="product_info">
                        <div className="product_info__categories">
                            {['description','video','size & specs','delivery & returns','reviews'].map((name,index) => {
                                return <button key={index} className={currentCategory === name ? 'active' : ''} onClick={() => setCurrentCategory(name)}>{name.toUpperCase()}</button>
                            })}
                        </div>
                        <div className="product_info__items">
                            {['description','video','size & specs','delivery & returns','reviews'].map((name,index) => {
                                return <div key={index} className={"product_info__item" + (currentCategory === name ? ' active' : '')} onClick={() => setCurrentCategory(name)}>{
                                    product[name as keyof ProductType] ? <>
                                        {name === 'description' && <>
                                            <h2>{product.description.title}</h2>
                                            {product.description.text.split('/backspace').map((p,index) => <p key={index}>{p}</p>)}
                                        </> }
                                    </> : <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Ad atque, consequatur doloribus enim est et facilis, inventore, iure magnam minima molestiae mollitia non optio perferendis placeat quidem unde veritatis vitae.</p>
                                }</div>
                            })}
                        </div>
                    </div>
